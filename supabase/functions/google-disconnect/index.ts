...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch the Google calendar connection for target user
    ...
    if (tokenToRevoke) {
      try {
        ...
        if (revokeResponse.ok) {
          ...
          revokeError = `Revoke failed: ${revokeResponse.status} - ${errorText}`;
          console.warn("Token revoke failed (proceeding with deletion):", revokeError);
        }
      } catch (err) {
        revokeError = `Revoke error: ${err instanceof Error ? err.message : String(err)}`;
        console.warn("Token revoke error (proceeding with deletion):", revokeError);
      }
    }
    ...
    console.log(`Google Calendar connection deleted for user ${targetInternalUserId}`);
